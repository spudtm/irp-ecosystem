...
public class UserResource implements RESTFulSPARQL
{
  ...
  public User create(User userDetails)
  {
    ...
    {
      userUrl = QueryReader.getString("UserQueries.baseNS") + UUID.randomUUID().toString();
                         ...
                         @DefaultValue("") @QueryParam("authenticationToken") String authenticationToken)
  {
    ...
    if(userUri.equals("") && authenticationToken.equals(""))
      throw new ExceptionReporter(new NullPointerException("'email' or 'userUri' or 'authenticationToken' is needed to delete the user."));
  ...
  public void update(User userDetails)
  {
    ...
    if(userDetails.getUserUri() == null || userDetails.getUserUri().trim().equals(""))
      throw new ExceptionReporter(new NullPointerException("User URI needed to update the user."));
    
    if(userDetails.getAuthenticationToken() == null || userDetails.getAuthenticationToken().trim().equals(""))
      throw new ExceptionReporter(new NullPointerException("Authentication token needed to update the user."));
    
    if(userDetails.getEmail() == null || userDetails.getEmail().trim().equals(""))
      throw new ExceptionReporter(new NullPointerException("Email needed to update the user."));
    
    if(userDetails.getPassword() == null || userDetails.getPassword().trim().equals(""))
      throw new ExceptionReporter(new NullPointerException("Password needed to update the user."));
    
    if(userDetails.getNickname() == null || userDetails.getNickname().trim().equals(""))
      throw new ExceptionReporter(new NullPointerException("Nickname needed to update the user."));
                                      ...
                                      @DefaultValue("") @QueryParam("password") String password)
  {
    ...
    if(email.equals(""))
      throw new ExceptionReporter(new NullPointerException("No 'email' given."));
    if(password.equals(""))
      throw new ExceptionReporter(new NullPointerException("No 'password' given."));
      
    String query = UserQueries.getIsValidLoginCredentialsQuery(email, password);
    Query sparqlQuery = new Query(query);
