...
@Path("/user-profiles")
@Scope("request")
public class UserProfilesResource implements RESTFulSPARQL
{
  @Context
  private UriInfo uriInfo;
  
  private SPARQLEndpoint userProfilesEndpoint;
  
  public void setUserProfilesEndpoint(SPARQLEndpoint userProfilesEndpoint)
  {
    this.userProfilesEndpoint = userProfilesEndpoint;
  }
  
  public SystemMessage init(ServiceInitialiser si)
  {
    return userProfilesEndpoint.init(uriInfo, si);
  }

  public void update(Query query)
  {
    userProfilesEndpoint.update(query);
  }

  public String query(Query query)
  {
    return Util.resultsetToString(userProfilesEndpoint.query(query));
  }

  public EndpointInfo info()
  {
    return userProfilesEndpoint.info();
  }
  ...
  public User create(User userDetails)
  {
    ...
    if(!userProfilesEndpoint.ask(sparqlQuery))
    {
      ...
      userProfilesEndpoint.update(sparqlQuery);
                  ...
                  @DefaultValue("") @QueryParam("authenticationToken") String authenticationToken)
  {
    ...
    Query sparqlQuery = new Query(query);
    ResultSet results = userProfilesEndpoint.query(sparqlQuery);
