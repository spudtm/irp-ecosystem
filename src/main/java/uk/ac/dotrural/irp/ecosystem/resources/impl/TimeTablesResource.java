...
@Path("/timetables")
@Scope("request")
public class TimeTablesResource implements RESTFulSPARQL
{
  @Context
  private UriInfo uriInfo;
  
  private SPARQLEndpoint timeTablesEndpoint;
  
  public void setTimeTablesEndpoint(SPARQLEndpoint timeTablesEndpoint)
  {
    this.timeTablesEndpoint = timeTablesEndpoint;
  }
  
  public SystemMessage init(ServiceInitialiser si)
  {
    return timeTablesEndpoint.init(uriInfo, si);
  }

  public void update(Query query)
  {
    timeTablesEndpoint.update(query);
  }

  public String query(Query query)
  {
    return Util.resultsetToString(timeTablesEndpoint.query(query));
  }

  public EndpointInfo info()
  {
    return timeTablesEndpoint.info();
  }
                                    ...
                                    @DefaultValue("false") @QueryParam("includeDirections") boolean includeDirections)
  {
    ...
    ResultSet results = timeTablesEndpoint.query(sparqlQuery);
  ...
  public Lines getServicesOnRoute(@DefaultValue("") @QueryParam("lineUri") String lineUri)
  {
    ...
    Query sparqlQuery = new Query(query);
    ResultSet servicesOnRoute = timeTablesEndpoint.query(sparqlQuery);
                                      ...
                                      @DefaultValue("inbound") @QueryParam("direction") String direction)
  {
    ...
    Query sparqlQuery = new Query(query);
    ResultSet busLocationsOnRoute = timeTablesEndpoint.query(sparqlQuery);
